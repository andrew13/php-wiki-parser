...
class WikiParser_WikiParser
{
    protected $parser_plugins = null;
    protected $parserType = 'mediawiki';

    protected static $config_ini     = null;
    ...
    public function __construct( $type = null )
    {
        ...
        $config = WikiParser_WikiParser::getConfigINI( $this->parserType );
        $isEnabled = true;
        $plugin_list = array();
    ...
    public static function getConfigINI( $type = null )
    {
        ...
        if( $type !== null )
        {
            switch( strtolower( $type ) )
            {
                case 'pmwiki':
                    $fileName = 'PMWiki.ini';
                    break;
                default: // default to mediawiki
                    $fileName = 'MediaWiki.ini';
            }
        }
        else
        {
        ...
        if(WikiParser_WikiParser::$config_ini === null)
        {
            WikiParser_WikiParser::$config_ini = parse_ini_file(dirname(__FILE__) . '/Config/' . $fileName, true);
        }
        return WikiParser_WikiParser::$config_ini;
    }
    ...
    public function parse($wiki_text)
    {
        ...
        $parser_order_config = WikiParser_WikiParser::getConfigINI();
